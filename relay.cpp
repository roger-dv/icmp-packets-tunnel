...
int relay(int pipe_out, sockaddr_un addr, [[maybe_unused]] socklen_t addr_len) {
...
//      logger->debug("{}(..): {} = select(..)", fn.data(), rc);
      if (rc == 0) {
        if (is_first) {
//          logger->debug("{}(..): Got no reply", fn.data());
        }
        break;
      } else if (rc < 0) {
        const auto ec = errno;
      ...
      if (rc < 0) { // check and handle error condition
        const auto ec = errno;
        logger->error("{}(..): recvfrom(__fd={},..): ec={}; {}", fn.data(), rcv_dst_sockfd, ec, strerror(ec));
        return EXIT_FAILURE;
      } else if (rc != 0) {
        if (static_cast<size_t>(rc) < sizeof(struct iphdr)) {
          ...
          if (rc < icmp_pck_len) {
            logger->warn("{}(..): got too short ICMP packet: {} bytes (expected minimum {} bytes)",
...
static int write_packet(const int fd, char *data, const size_t datasize, const sockaddr& addr, const socklen_t addr_len) {
  ...
  if (n < 0) {
    const auto ec = errno;
    logger->error("{}(..): write(__fd={}..): ec={}; {}", __FUNCTION__, fd, ec, strerror(ec));
    return EXIT_FAILURE;
  }
